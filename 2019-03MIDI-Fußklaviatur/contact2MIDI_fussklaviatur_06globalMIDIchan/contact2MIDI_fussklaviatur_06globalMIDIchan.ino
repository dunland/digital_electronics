...
void loop() {
  ...
  if (digitalRead(up) == LOW && millis() - lastDebounceTime > debounceDelay && lastUpState == HIGH)
  {
    if (shift < 72)
    {
      shift += 12;
  ...
  if (digitalRead(down) == LOW && millis() - lastDebounceTime > debounceDelay && lastDownState == HIGH)
  {
    if (shift > 0)
    {
      shift -= 12;
  ...
  lastDownState = digitalRead(down);

  /* ------------------------------ CHANNEL SWITCH --------------------------- */
  if (digitalRead(down) == LOW && digitalRead(up) == LOW)
  {
    globalMidiChannel = (globalMidiChannel + 1) % 16;
    delay(1000); // TODO: besseren debounce hier.
  }
  ...
  for (int i = 0; i < 13; i++)
  {
    if (digitalRead(pins[i]) == LOW && !noteOn[i])
    {
      ...
      MIDI.sendNoteOn(note[i], 127, globalMidiChannel);
    }
  }
  ...
  for (int i = 0; i < 13; i++)
  {
    if (digitalRead(pins[i]) == HIGH)
    {
      noteOn[i] = false;
      digitalWrite(13, LOW);
      MIDI.sendNoteOff(note[i], 127, globalMidiChannel);
      note[i] = noteBase[i] + shift;
    }
  }
}
